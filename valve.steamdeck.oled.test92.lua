...
gamescope.config.known_displays.steamdeck_oled_sdc_test = {
    ...
    dynamic_modegen = function(base_mode, refresh)
        ...
        -- For 92-96Hz, try using minimum VFP (9) like stock 90Hz
        -- and let calc_max_clock figure out the clock
        if refresh >= 92 and refresh <= 96 then
            debug("TEST: Trying "..refresh.."Hz with VFP=9 and calculated clock")
            gamescope.modegen.adjust_front_porch(mode, 9)
            mode.clock = gamescope.modegen.calc_max_clock(mode, refresh)
            mode.vrefresh = refresh
            debug("TEST: "..refresh.."Hz clock="..mode.clock)
            return mode
        end
...
gamescope.config.known_displays.steamdeck_oled_boe_test = {
    ...
    dynamic_modegen = function(base_mode, refresh)
        ...
        -- For 92-96Hz, try using minimum VFP (8) and calculated clock
        if refresh >= 92 and refresh <= 96 then
            debug("TEST: Trying "..refresh.."Hz with VFP=8 and calculated clock")
